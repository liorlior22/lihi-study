...
function updateBankSummary(status='V9 · vocab-bank.json'){
...
async function init(){
  try{
    ...
    renderLetters();updateCounts();updateBankSummary(`V9 · ${WORDS.length.toLocaleString()} translated entries · vocab-bank.json`);
  }catch(error){
    console.error('[English 100 V9]',error);
    WORDS=[];renderLetters();updateCounts();updateBankSummary(`V9 load failed · ${error.message}`);
  }
}
