...
function updateBankSummary(status='V8 · vocab-bank.json'){
...
async function init(){
  try{
    ...
    renderLetters();updateCounts();updateBankSummary(`V8 · ${WORDS.length.toLocaleString()} translated entries · vocab-bank.json`);
  }catch(error){
    console.error('[English 100 V8]',error);
    WORDS=[];renderLetters();updateCounts();updateBankSummary(`V8 load failed · ${error.message}`);
  }
}
