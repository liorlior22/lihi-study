...
function updateBankSummary(status='V10 · vocab-bank.json'){
...
async function init(){
  try{
    ...
    renderLetters();updateCounts();updateBankSummary(`V10 · ${WORDS.length.toLocaleString()} translated entries · vocab-bank.json`);
  }catch(error){
    console.error('[English 100 V10]',error);
    WORDS=[];renderLetters();updateCounts();updateBankSummary(`V10 load failed · ${error.message}`);
  }
}
