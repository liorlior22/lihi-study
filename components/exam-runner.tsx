...
export function ExamRunner({ questions, title, sessionKey }: { questions: ExamQuestion[]; title: string; sessionKey: string }) {
  ...
  const currentStudyId=studyIdForExamTopic(current.topic),correctOption=correctOptionFor(current);return <section className="exam-runner"><header><div><small>{title}</small><b>שאלה {index + 1} מתוך {ordered.length}</b></div><span>{Math.round((index / ordered.length) * 100)}%</span></header><div className="exam-progress"><i style={{ width: `${((index + 1) / ordered.length) * 100}%` }} /></div><article className="question-card"><div className="question-meta"><span>שאלה {current.questionNumber}</span><span>{current.topic}</span></div><h1>{cleanExamText(current.question)}</h1>{current.image&&<div className="exam-question-image"><Image src={current.image} alt={`תרשים לשאלה ${current.questionNumber}`} width={720} height={720}/></div>}<div className="answer-list">{current.options.map(o => { let state = selected === o.id ? "selected" : ""; if (checked && o.id === current.correctAnswer) state = "correct"; else if (checked && selected === o.id) state = "incorrect"; return <button disabled={checked} className={state} key={o.id} onClick={() => setSelected(o.id)}><b>{o.id}</b><span>{cleanExamText(o.text)}</span></button> })}</div>{checked && <div className={`answer-feedback ${selected === current.correctAnswer ? "right" : "wrong"}`}>{selected === current.correctAnswer ? CORRECT : WRONG}{selected !== current.correctAnswer&&<p><strong>התשובה הנכונה: {current.correctAnswer}. {cleanExamText(correctOption?.text ?? "")}</strong></p>}<p><strong>הסבר:</strong> {explanationFor(current)}</p>{currentStudyId&&<Link href={`/study/${currentStudyId}`}>לקטע הלימוד המלא ←</Link>}</div>}<div className="question-actions dual-actions"><button disabled={!selected || checked} onClick={immediateCheck}>בדיקת תשובה</button><button className="next-unchecked" onClick={next}>שאלה הבאה</button></div></article></section>;
}
