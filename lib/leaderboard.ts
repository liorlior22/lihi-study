...
async function request(path:string,init?:RequestInit){const{url,key}=config();const headers:Record<string,string>={apikey:key,"Content-Type":"application/json",Prefer:"return=representation"};if(!key.startsWith("sb_secret_"))headers.Authorization=`Bearer ${key}`;const response=await fetch(`${url}/rest/v1/${path}`,{...init,headers:{...headers,...init?.headers},cache:"no-store"});const body=await response.text();if(!response.ok){console.error("Supabase REST error",response.status,body);throw new Error(`Database request failed: ${response.status}`)}return body?JSON.parse(body):null}
