...
export default function Home() {
  return <><RedHeader/><main className="dashboard-page">
    ...
    <section className="dashboard-grid">{cards.map(card => <Link href={card.href} key={card.title}><span>{card.icon}</span><div><h2>{card.title}</h2><p>{card.text}</p></div><i>←</i></Link>)}</section>
